...
// Task dependencies are executed in parallel
gulp.task('three', ['one', 'two'], function() {})

gulp.task('parallel', function () {
    // Tasks provided as an array are executed in parallel
    runsequence(['one', 'two'])
})

gulp.task('series', function (done) {
    // Tasks provided as separate args are excuted in series - as long as a 'done' parameter is passed through the task
    // and passed into runsequence as the final task to be run - this is used by Gulp to know when the task has
    // finished.
    runsequence('one', 'two', done)
})
